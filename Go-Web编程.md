...
要想写出独立性较好的测试，需要使用测试替身进行解耦。这就要求程序员在编码前进行相应的设计。

实现测试替身的一种设计方法是使用依赖注入的设计模式。解耦依赖分为两种，一种是自底向上，另一种是自顶向下。

GoCheck在testging的基础上，提供了更丰富的测试功能。

测试驱动开发(BDD)是行为驱动开发(TDD)的延伸，是一种软件开发方法而不是一种软件测试方法。Go中可用于BDD的第三方是Ginkgo。

# 第九章 并发

介绍并发特性，学会如何利用并发特性来提高性能。

“并发指的是同时处理多项任务，而并行指的是同时执行多项任务。”——Rob Pike, Go语言的作者之一

Go语言通过goroutine和通道这两个主要组件来为并发提供支持。

goroutine是对线程的多路复用。goroutine很轻量级，因此它的数量可以比线程多很多。一个goroutine在启动时，只需要一个非常小的栈（在Go1.4中，仅为2KB），并且这个栈可以扩展和缩小。

当一个goroutine被阻塞时，它也会阻塞所服用的操作系统线程，而运行时环境(runtime)则会把位于被阻塞线程上的其他goroutine移动到其他未阻塞的线程上继续运行。

虽然goroutine很轻量级，但是它的调度也是有代价的。因此，有时候并发反而会比串行慢。

同理，使用更多的CPU进行并发也不一定会更快，因为对更多的CPU进行调度和运行任务需要耗费更多的资源。

因此，当想使用并发提高性能时，应该使用Benchmark进行基准测试，了解它的性能。
...
| 名词                              | 含义                                     |
| --------------------------------- | ---------------------------------------- |
| IaaS(Infrastructure-as-a-Service) | 提供计算、存储以及网络在内的基本计算能力 |
| PaaS(Platform-as-a-Service)       | 向用户提供工具部署应用                   |
| SaaS(Software-as-a-Service)       | 向用户提供应用服务                       |

使用PaaS来部署Web应用，可以让基础设施和系统层变得抽象，并且不需要人工的管理和干预。

为了持续运行进程，需要用到upstart或者systemd这样的init守护进程：init进程是类Unix系统在启动时运行的第一个进程，该进程由内核负责启动，它会一直运行直到系统关闭为止，并且它还是其他所有进程直接或间接的祖先。

虚拟机：模拟包括操作系统在内的整个计算机系统。

容器：提供操作系统级别的虚拟化，并将资源划分给多个用户空间实例。

通过使用docker，可以很方便地部署web应用，这一方式变得越来越流行。

不同的部署方式，有各自的优缺点，包括代码的修改量、配置系统的工作量、系统的维护、部署难度、可拓展性、平台支持程度、应用与平台的紧密程等方面。